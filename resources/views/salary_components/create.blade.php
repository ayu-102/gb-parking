<x-app-layout>
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-slate-800 tracking-tight">Tambah Komponen Gaji Baru</h1>
                <p class="text-xs text-slate-500 mt-1">Buat komponen tunjangan atau potongan standar baru.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <form action="{{ route('salary-components.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Kode
                        Komponen</label>
                    <input type="text" name="code" value="{{ old('code') }}"
                        placeholder="Contoh: TJ-MAKAN, POT-BPJS" required
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}"
                        placeholder="Contoh: Tunjangan Makan & Transport" required
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" required
                            ...
                            <option value="allowance" {{ old('type') == 'allowance' ? 'selected' : '' }}>Pendapatan /
                                Tunjangan (+)</option>
                    ...
                    <div>
                        ...
                        <select name="amount_type" required
                            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#FF6B00] bg-white font-medium">
                            <option value="fixed" {{ old('amount_type') == 'fixed' ? 'selected' : '' }}>Nominal Tetap
                                (Rp)</option>
                            <option value="percentage" {{ old('amount_type') == 'percentage' ? 'selected' : '' }}>
                                Persentase (%)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nilai / Nominal
                        Default</label>
                    <input type="number" step="0.01" name="amount" value="{{ old('amount', 0) }}" placeholder="0"
                        required
                    ...
                    <p class="text-[10px] text-slate-400 mt-1">Isi angka nominal (misal: 250000) atau nilai persen
                        (misal: 1 untuk 1%).</p>
                </div>
