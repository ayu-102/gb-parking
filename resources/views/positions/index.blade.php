<x-app-layout>
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Master Data Jabatan</h1>
                <p class="text-xs text-slate-500 mt-1">Kelola daftar struktur posisi dan jenjang posisi kerja GB Parking.
                </p>
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-3.5">
                ...
                <div>
                    ...
                    <h4 class="text-base font-extrabold text-slate-800 tracking-tight">
                        {{ number_format($totalPositions) }} Posisi</h4>
                </div>
            </div>
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-3.5">
                ...
                <div>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Terhubung Departemen</p>
                    <h4 class="text-base font-extrabold text-slate-800 tracking-tight">
                        {{ number_format($assignedDepartments) }} Jabatan</h4>
                </div>
            </div>

            <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-3.5">
                <div
                    class="w-11 h-11 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center font-bold text-base shrink-0">
                    <i class="fa-solid fa-layer-group"></i>
                </div>
                <div>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tanpa Departemen</p>
                    <h4 class="text-base font-extrabold text-slate-800 tracking-tight">{{ number_format($unassigned) }}
                        Jabatan</h4>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-4 border-b border-slate-100 flex items-center justify-between gap-4 bg-slate-50/50">
                <form method="GET" action="{{ route('positions.index') }}" class="relative w-full sm:w-80">
                ...
                <table class="w-full text-left text-xs">
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="p-4">Karyawan Active</th>
                            ...
                            <tr class="hover:bg-slate-50/80 transition">
                                <td class="p-4">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="font-bold text-slate-800 text-xs">{{ $position->name }}</span>
                                    </div>
                                </td>
                                <td class="p-4">
                                    @if ($position->department)
                                        <span
                                            class="px-2.5 py-1 bg-blue-50 text-blue-600 font-bold rounded-lg text-[10px]">
                                            <i
                                                class="fa-solid fa-building text-[9px] mr-1"></i>{{ $position->department->name }}
                                        </span>
                                    @else
                                        <span
                                            class="px-2.5 py-1 bg-slate-100 text-slate-400 font-semibold rounded-lg text-[10px]">
                                            Tanpa Departemen
                                        </span>
                                    @endif
                                </td>
                                <td class="p-4 font-semibold text-slate-600">
                                    <span
                                        ...
                                        {{ $position->employees_count ?? 0 }} Orang
                                    </span>
                                </td>
                                <td class="p-4 text-center">
                                    <div class="flex items-center justify-center space-x-1">
                                        ...
                                        <form action="{{ route('positions.destroy', $position->id) }}" method="POST"
                                            class="inline-block"
                                            onsubmit="return confirm('Yakin ingin menghapus jabatan {{ $position->name }}?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="p-12 text-center text-slate-400">
                                    ...
                                    <p class="font-bold text-slate-600 text-sm">Belum Ada Data Jabatan</p>
                                    <p class="text-xs text-slate-400 mt-1">Klik tombol 'Tambah Jabatan' di atas untuk
                                        memasukkan data posisi baru.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
