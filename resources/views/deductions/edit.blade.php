<x-app-layout>
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-slate-800">Edit Potongan Gaji</h1>
                <p class="text-xs text-slate-500 mt-1">Perbarui data potongan karyawan.</p>
            </div>
            ...
            <form action="{{ route('deductions.update', $deduction->id) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="employee_id" required
                        ...
                        @foreach ($employees as $emp)
                            <option value="{{ $emp->id }}"
                                {{ $deduction->employee_id == $emp->id ? 'selected' : '' }}>{{ $emp->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Jenis
                        Potongan</label>
                    <input type="text" name="type" value="{{ old('type', $deduction->type) }}" required
                        class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#FF6B00]">
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Jumlah
                            Potongan (Rp)</label>
                        <input type="number" name="amount" value="{{ old('amount', $deduction->amount) }}" required
                            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#FF6B00]">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $deduction->date) }}" required
                            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#FF6B00]">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#FF6B00]">{{ old('description', $deduction->description) }}</textarea>
                </div>
