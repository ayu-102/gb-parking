<x-app-layout>
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-slate-800 tracking-tight">Edit Absensi Karyawan</h1>
                <p class="text-xs text-slate-500 mt-1">Koreksi jam atau status kehadiran.</p>
            </div>
            ...
            <form action="{{ route('attendances.update', $attendance->id) }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" required
                            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#FF6B00] font-medium bg-white">
                            <option value="present" {{ $attendance->status == 'present' ? 'selected' : '' }}>Hadir
                                (Tepat Waktu)</option>
                            <option value="late" {{ $attendance->status == 'late' ? 'selected' : '' }}>Terlambat
                            </option>
                            ...
                            <option value="absent" {{ $attendance->status == 'absent' ? 'selected' : '' }}>Alpha /
                                Mangkir</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#FF6B00]">{{ old('notes', $attendance->notes) }}</textarea>
                </div>
